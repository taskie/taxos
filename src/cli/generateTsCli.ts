...
const utilMapperGenerator = (apiName: string, swagger: Swagger, src: string) => ({
  [`${src}/api/utils/objectToFormData.ts`]: generateObjectToFormDataTs,
  [`${src}/api/${apiName}/utils/apiContext.ts`]: () => generateApiContextTs(swagger),
});
...
export default function generateTsCli(opts: Options) {
  ...
  const srcOutDir = src != null ? src : "src";
  ...
  for (let defPath of defPaths) {
    ...
    const basePath = path.dirname(path.join(srcOutDir, "api", defPath.slice(swaggerOutDir.length)));
  ...
  for (let pathPath of pathPaths) {
    ...
    const basePath = path.dirname(path.join(srcOutDir, "api", pathPath.slice(swaggerOutDir.length)));
